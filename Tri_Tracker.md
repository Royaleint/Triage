...
- **Rebuild spec progress (2026-04-26):** Codex authored `Triage_Dev/plans/active/triage-rebuild-architecture-spec.md` (959 lines first pass; revised to 1217 lines after Claude review). Substantive revisions: B-alt1 strategic positioning cost made explicit + B-alt2 recommended as default unless Rawb accepts custom-frame-surface shift; SavedVariables migration deferred to dedicated sub-plan with itemized requirements (current `DATABASE_VERSION = 2.3`, flat profile keys catalogued); 8-phase sequence reworked into 9 user-value-first "Slices" (Slice 1 = Retail Dispel Source Refactor, capability layer demoted to Slice 2); minimal-changes alternative added; pre-marked decisions converted to "Rawb decision needed"; 6-item Rawb Decision Gates section added with recommendations + impact; TBC Classic clarified as out-of-scope unless Rawb opts in; headless test harness explicitly net-new infrastructure requiring own plan. Spec is now architecture-overview shape (each implementation slice will get its own Gate 0/1/2/3/4 plan).
- **Decision Gates resolved (2026-04-26):** Rawb resolved all six Gates. **Q1:** Alternative C — ERF-preserving rebuild, value-sliced. **Q2:** B-alt1 first (own `SecureUnitButtonTemplate` clone) — Rawb override of Codex/Claude B-alt2 recommendation; positioning shift accepted (Triage owns boss-frame rendering surface for boss units only). **Q3:** Defer TBC Classic. **Q4:** Move drag-and-drop binding UX to TRI-005 plan. **Q5:** Active-only dispel signal first; whitelist scaffolding ships in Slice 1. **Q6:** Defer TRI-009/TRI-010 priority engine until whitelist + degrade rules tested. Architecture spec updated on disk to reflect resolutions (Decision Status note added to Document Status; Decisions table updated; Rawb Decision Gates section now records resolutions; Boss Frame Policy updated to note B-alt1 selection and positioning shift; Slice 6 updated to reflect B-alt1 first).
- **Slice 1 sub-plan progress (2026-04-26):** Prodigy authored TRI-035 Slice 1 (Retail Dispel Source Refactor) sub-plan in chat following `BawrLabs/plans/PLAN_TEMPLATE.md` Gate 0/1/2/3/4 structure. Scope: replace `frame.dispels` reads in `Modules/DispelOverlay.lua` (lines 271, 283) with new `Modules/DispelSource.lua` reader using `frame.DispelOverlay.dispelDebuffFrames[i].aura` + `issecretvalue` guards; introduce minimal capability lookup (4 flags) future-friendly for Slice 2 extraction; ship empty `Data/DispelWhitelist.lua` Layer 4 schema scaffolding; preserve Mists/Classic Era legacy path unchanged; active-only baseline per Q5; refactor-contract Tier 3 compliance. 5 files (2 new, 3 modified). 3 commits + optional 4th for `/tridev dispelcheck` diagnostic. **Plan written to disk (2026-04-26):** `Triage_Dev/plans/active/tri-035-slice-1-dispel-source.md` with all 6 Gate 3 questions resolved (Rawb quick-yes plan defaults).
- **TRI-036 sub-plan progress (2026-04-26):** Prodigy authored TRI-036 (ERF → Triage internal rename) sub-plan. Pure refactor + Tier 3. 26 files, 5 commits, ~340 anchored renames + ~25 inline frozen-string comments. `_G.Triage = _G.EnhancedRaidFrames` alias preserves AceAddon binding (Triage_Dev hooks unchanged), AceLocale paired registry frozen across 20 sites, LibDualSpec namespace frozen (preserves dual-spec data), LibClassicDurations registration frozen. All 6 Gate 3 questions resolved (Q5 evidence-based via external-reader research: zero documented external readers of `frame.ERF_*` in public corpus). Plan on disk at `Triage_Dev/plans/active/tri-036-erf-rename.md`. Sequencing: lands AFTER TRI-035 Slice 1 merges to main.
- **Slice 1 implementation + Gate 1 (2026-04-26):** Codex implemented Slice 1 in worktree `.worktrees/tri-035-retail-dispel-source/`. **First Argus Gate 1 verdict: BLOCKED** — multi-slot short-circuit bug + spec deviation (inline implementation instead of separate `DispelSource.lua` + `DispelWhitelist.lua`). Codex addressed both round-trips: (1) Created `Modules/DispelSource.lua` with `GetActiveDispelType` + `GetDispelCapabilities` + multi-slot reader collecting all readable types then resolving via `PRIORITY_ORDER`. (2) Created `Data/DispelWhitelist.lua` empty schema scaffold. (3) Updated `DispelOverlay.lua` to consume the new public method with explicit unknown-active branch. **Second Argus Gate 1 verdict: PASS** — all four prior findings (F1.1-F1.4) verified fixed; spec deviation closed. Three optional improvements (closure allocation, dead caps gate, explicit consumer branch) all addressed in follow-up commits. Argus's Lens 5 `pcall`-on-secret-fields documentation gap closed via Section 6 added to `Triage/.claude/rules/aura-secrets.md` (junctioned to `Triage_Dev/.claude/rules/aura-secrets.md`).
- **Slice 1 commits + merge (2026-04-26):** Public worktree branch `tri-035-slice-1-dispel-source` has 2 commits: `9b89189 feat(dispel): add DispelSource module and DispelWhitelist scaffolding` + `a38191a feat(dispel): route DispelOverlay through DispelSource`. Triage_Dev master has 2 commits: `cf01493 docs(rules): document pcall pattern for private-aura overlay field reads` + `33c5e53 feat(tridev): extend /tridev dispelcheck for Slice 1 verification`. **Merged to main at `6860c93`** via `git merge --no-ff` per `/gate2-prep` protocol.
- **Slice 1 Gate 2 PARTIAL PASS (2026-04-26):** Dispel logic verified correct via `/tridev dispelcheck` live data — capabilities report correctly (`blizzOverlay=true`, `readableFields=conditional`, `legacyFrameDispels=false`), multi-slot reader correctly identified `active=Magic` for party members with Magic debuffs while filtering out non-dispellable Bleeds (validates F1.1-F1.4 fixes under live data). **However, visual rendering blocked by pre-existing combat-lockdown bug**: `ADDON_ACTION_BLOCKED` fired in follower dungeon at `Modules/AuraIndicators.lua:186` (`SetPropagateMouseClicks`), tainting the `CompactUnitFrame_UpdateAuras` execution chain and causing `rendered=nil` despite correct `active=Magic` reads. Bug is pre-existing (`8136e8a3`, 2026-04-06) — TRI-035 surfaced but did not cause it.
- **TRI-037 implementation + Gate 1 (2026-04-26):** Codex implemented TRI-037 directly on main (worktree skip authorized by Rawb). **Cycle-1 Argus Gate 1: BLOCKED** — AceEvent-3.0 collision at `EnhancedRaidFrames.lua:120` vs `Overrides.lua:58` (both registered `PLAYER_REGEN_ENABLED` on the same addon object; CallbackHandler-1.0 stores one callback per `(self, event)` pair; flush handler silently overwritten on first `GROUP_ROSTER_UPDATE` post-login). Codex consolidated both side-effects into single registration in `OnEnable()` and removed duplicate from `Overrides.lua:49`. **Cycle-2 Argus Gate 1: PASS** — single `PLAYER_REGEN_ENABLED` registration confirmed; both side-effects (FlushDeferredMouseBehavior + pending stock-aura visibility update) run; flag-set/check/clear lifecycle intact; nothing lost from original `EnsureRetailStockAuraVisibilityEvents` (PLAYER_ENTERING_WORLD registration preserved). All 5 lenses PASS. Cleared for commit on main.
- **TRI-035 + TRI-037 closure decision (2026-04-26):** Rawb authorized risk-accepted closure given <40 user base. Retail Gate 2 effectively passes once TRI-037 commits (the same session that proved dispel logic correct will retest as the Retail Gate 2 pass post-TRI-037 unblocking the taint chain). Mists Classic + Classic Era regression testing **deferred to community testers** — defensible because: (1) Argus confirmed legacy `frame.dispels` reader semantics are byte-identical to v1.1.x; (2) the dispatch routes Classic clients to legacy branch via `caps.supportsLegacyFrameDispels = true`; (3) Retail-only secret-guards never execute on clients where `issecretvalue` doesn't exist. CHANGELOG should ask explicitly for Classic testers to report any dispel-overlay regression. **Next action:** Codex commits TRI-037 (3 files: `EnhancedRaidFrames.lua`, `Modules/AuraIndicators.lua`, `Overrides.lua`) — Tri_Tracker.md stays separate. Rawb retests Retail M1-M3 multi-debuff scenario post-commit (5-min repro, Argus-required). Both TRI-035 + TRI-037 close on M1-M3 PASS.
...
- **Status:** Queued — pending Codex implementation
...
- **Session progress (2026-04-26, rebuild verification):**
  ...
  - **GitHub issue updates posted.** Codex's connector returned 403 earlier; Claude Code's `gh` CLI auth was working as Royaleint with `repo` scope, so all 12 prepared comments from `Triage_Dev/plans/active/github-issue-finding-updates-2026-04-26.md` were posted to issues `#1`, `#4`, `#5`, `#8`, `#9`, `#13`, `#17`, `#18`, `#24`, `#25`, `#27`, `#41`. Each returned a valid issue-comment URL.
  - **TRI-006 v1.1.0 obsolescence cross-check:** independently verified each spell ID in `Data/SpecDefaults.lua` against current 12.0.5 sources (Wowhead per-spell), without using Cell PR #457's removed-spells list as a source. Findings: (1) Renew (139) in Holy Priest slot [1] is stale — removed as baseline castable in Midnight 12.0; only available via specific talents now. (2) Soothing Mist (1260617) duplicated in Mistweaver slots [2] and [6] — likely one-line bug. (3) Plea (200829) addition Cell made for Disc — recommended NO change, doesn't fit Triage's HoT/buff slot model. Triage's tighter curation already avoided 8 of 9 Cell-removed spells. Full report in `Triage_Dev/plans/active/triage-rebuild-tri006-obsolescence-check.md`.
  - **TBC Classic Anniversary cost assessment complete (added as TRI-034).** TBC Anniversary launched January 2026 with modern Midnight 12.0+ backend; Prunes ERF for TBC Anniversary (1.5K+ downloads) confirms CompactUnitFrame APIs available. Library compat verified — LibRangeCheck and LibDispel both have TBC branches; LibClassicDurations and LibDualSpec do not apply. Effort estimate 2-4 days static + community-assisted verification. Sequencing: after TRI-001a Spike B Gate 2 and TRI-005 Spike A; can run in parallel with rebuild architecture work. Open product call: ship in v1.x point release vs bundle with v2.x rebuild release. Full assessment in `Triage_Dev/plans/active/triage-rebuild-tbc-cost-assessment.md`.
  - **Cell PR #457 pattern extraction (no contamination):** identified 9 architectural patterns safe to learn from (per-field secret guards, whitelist helper trio, dual-path init via single capability flag, CLEU substitution via `UNIT_AURA`/`UNIT_HEALTH`, comm restriction multi-API check, API existence guards, dedicated heal-prediction calculator for previews, local fallback for removed Blizzard globals, version-gated migration code) and 3 contamination zones to NOT absorb (538-spell `RaidDebuffs_Midnight.lua` content, spec-defaults removal/addition lists, specific spell IDs Cell hardcodes as known-non-secret). Patterns yes, content no. Cross-validation: Cell shipped almost exactly the architectural shape Codex's confidence-gates doc describes — strong external validation that the approved hybrid + capability-flag direction is correct.
  - **MidnightHealerUI live build review:** verified the CurseForge v1.2.0 build differs materially from the GitHub `Fatalcaleb/MidnightHealer` v0.12.8 repo (different file structure, different author name in TOC). Live build classification: Approach C (Custom UnitButton) at minimal scale, Ace3 essentials + custom ProfilDB. Verified: 5-language localization is real (`Locales/{enUS,frFR,deDE,esES,ruRU}.lua` all present), `UnitElements.lua` exists, `Cluster.lua` is GitHub-only (live build doesn't ship spatial cluster finder — TRI-011 differentiator restored). Combat-deferral surface: 6 sites total via centralized `pendingBindings`/`pendingRosterUpdate` flags drained on `PLAYER_REGEN_ENABLED` (vs Danders' 306 scattered) — this is the architectural lesson worth applying to TRI-005. Anti-patterns: aggressive `UnregisterAllEvents + Hide + SetParent(HiddenFrame)` triple-action when hiding Blizzard frames, 5Hz polling for range, under-developed secret-value surface (2 sites). Full review in `Triage_Dev/plans/active/triage-rebuild-mhu-architecture-review.md`.
  - **Plans-doc terminology cleanup:** swept `Triage_Dev/plans/active/` for legacy "backlog" prose references and updated to point at `Tri_Tracker.md`. Convention is documented in Triage `CLAUDE.md`; my prose just slipped on it. Fixed in `triage-rebuild-cell-mhu-research.md` and `triage-rebuild-ingame-verification.md`.
  ...
  - **Capability flags:** Capability layer should replace scattered client/frame assumptions. Current derived shape includes UnitAura payload support, LibClassicDurations need, LibDualSpec support, compact party frames, mouse input propagation, private-aura suppression/hooking, spec defaults, Retail indicator features, Blizzard dispel-overlay state, readable aura-field state, LibDispel player capability, and Triage-owned click-cast support. `supportsFrameDispels` should be removed/deprecated.
  - **Architecture spec drafted:** Combined Codex and Claude Code findings into `Triage_Dev/plans/active/triage-rebuild-architecture-spec.md`. The draft locks in ERF-preserving rebuild as the default path, defines BlizzardCompactProvider / TriageOwnedSecureProvider / unresolved BossFrameProvider, rejects the current `CompactUnitFrameTemplate` boss path, and sequences capability layer -> provider boundary -> frame state store -> dispel source refactor -> click-cast/boss replacement spikes.
- **Pending (next session):**
  ...
  - Review `Triage_Dev/plans/active/triage-rebuild-architecture-spec.md` with Claude Code and resolve open product calls: B-alt1/B-alt2 timing, TBC v1.x vs v2.x, TRI-005 drag-and-drop UX, and minimum viable dispel urgency.
  - Revise TRI-001 boss-frame architecture before any further live boss testing. Do not rerun the current `CompactUnitFrameTemplate` boss prototype except as an explicit unsafe retest. Replace old B1-B6 with B-alt1 (`SecureUnitButtonTemplate` visual clone) and/or B-alt2 (read-only Blizzard boss-frame overlay) gates.
